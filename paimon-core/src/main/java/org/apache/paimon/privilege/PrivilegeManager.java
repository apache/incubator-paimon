...
/**
 * Manages user and privilege system tables.
 *
 * <p>There are two special users: root and anonymous. Root is the user with all privileges, and
 * anonymous is the default username if no user and password are provided when creating the catalog.
 *
 * <p>User table schema: user (primary key), sha256 of password.
 *
 * <p>Privilege table schema: user or role name (primary key), entity type (primary key), identifier
 * of table or database (primary key), privilege type (primary key).
 *
 * <p>If a user has a privilege on an identifier A, he also has this privilege on identifier B where
 * A is a prefix of B.
 */
public class PrivilegeManager {
    ...
    public PrivilegeManager(String warehouse, FileIO fileIO, String user, String password) {
    ...
    public void initializePrivilege(String rootPassword) {
        if (getUserTable(false) != null && getPrivilegeTable(false) != null) {
    ...
    public void objectRenamed(String oldName, String newName) {
        Table privilegeTable = getPrivilegeTable(false);
    ...
    private void assertUserPassword() {
        Table userTable = getUserTable(false);
        if (userTable == null) {
            return;
        }
